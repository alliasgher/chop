...
interface Props {
  ...
  searchParams: Promise<{ staffId?: string; serviceId?: string; startsAt?: string }>;
}

export default async function DetailsPage({ params, searchParams }: Props) {
  const { shopSlug } = await params;
  const { staffId, serviceId, startsAt } = await searchParams;

  if (!staffId || !serviceId || !startsAt) notFound();
            ...
            <div className="bg-white rounded-2xl border-2 p-6 space-y-4" style={{ borderColor: `${shop.colors.primary}12` }}>
              ...
              <div className="space-y-3 text-sm">
                <div className="flex justify-between">
                  <span style={{ color: `${shop.colors.primary}60` }}>Barber</span>
                  <span className="font-medium" style={{ color: shop.colors.primary }}>{barber.name}</span>
                </div>
                <div className="flex justify-between">
                  <span style={{ color: `${shop.colors.primary}60` }}>Service</span>
                  <span className="font-medium" style={{ color: shop.colors.primary }}>{service.name}</span>
                </div>
                <div className="flex justify-between">
                  <span style={{ color: `${shop.colors.primary}60` }}>Duration</span>
                  <span className="font-medium" style={{ color: shop.colors.primary }}>{service.duration_min} min</span>
                </div>
                <div className="flex justify-between">
                  <span style={{ color: `${shop.colors.primary}60` }}>Time</span>
                  <span className="font-medium text-right" style={{ color: shop.colors.primary }}>{formattedTime}</span>
                </div>
                <div className="border-t pt-3" style={{ borderColor: `${shop.colors.primary}15` }}>
                  <div className="flex justify-between font-semibold">
                    <span style={{ color: shop.colors.primary }}>Deposit due</span>
                    <span style={{ color: shop.colors.accent }}>${(service.deposit_cents / 100).toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between mt-1 text-xs" style={{ color: `${shop.colors.primary}50` }}>
              ...
              <div className="rounded-lg px-3 py-2 text-xs" style={{ backgroundColor: `${shop.colors.accent}10`, color: shop.colors.accent }}>
                Demo mode — no money moves. Card pre-filled.
              </div>
            </div>
          </div>
